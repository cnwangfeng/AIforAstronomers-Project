...
- Part II 最新补强已加入共同公式地图，并加厚距离模数推导、孔径测光净信号推导、周期搜索线性系数到振幅/相位的解释。
...
- `bash scripts/build_book_local.sh zh`：通过，中文 PDF 输出为 `/tmp/aifor_book_main_zh/main_zh.pdf`，页数 `483`。
...
- 最新细读补强已加入 Part II 共同公式地图，并在 `ch11`、`ch12`、`ch14` 中分别补入距离模数来源、孔径测光净信号来源、周期模型线性系数如何回到振幅和相位的教材化解释。
...
- `bash scripts/build_book_local.sh zh`：通过（当前中文书稿 `483` 页）
- `git diff --check`：通过
