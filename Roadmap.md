...
- 本轮已在完整项目仓库运行 `python scripts/smoke_test_notebooks.py`、`python scripts/validate_data_manifest.py`、`bash scripts/build_book_local.sh main`、`bash scripts/build_book_local.sh zh`、中文 LaTeX 错误扫描和 `git diff --check`。
- 如需同步书稿，先运行 `bash scripts/sync_book_to_overleaf.sh` dry-run，再经确认运行 `bash scripts/sync_book_to_overleaf.sh --apply`。
