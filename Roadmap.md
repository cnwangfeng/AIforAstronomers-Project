...
- `bash scripts/run_release_checks.sh quick`：通过，包含 data manifest、release inventory、中文 LaTeX log、`git diff --check` 和 Overleaf dry-run payload 检查；当前 dry-run payload 为 `104` 个文件。
- `bash scripts/run_release_checks.sh full`：通过，完整复跑 data manifest、release inventory、`59` 个 notebook smoke test、中英文 LaTeX 构建、中文 LaTeX log、`git diff --check` 和 Overleaf dry-run payload 检查。
- `git diff --check`：通过。
...
- 本轮已在完整项目仓库运行 `bash scripts/run_release_checks.sh full`，完整复跑 `python scripts/smoke_test_notebooks.py`、`python scripts/validate_data_manifest.py`、`bash scripts/build_book_local.sh main`、`bash scripts/build_book_local.sh zh`、`python scripts/check_latex_log.py`、`git diff --check` 和 Overleaf dry-run payload guard；当前结果通过。
