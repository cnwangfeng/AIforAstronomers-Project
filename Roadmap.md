...
- `bash scripts/build_book_local.sh zh`：通过，中文 PDF 输出为 `/tmp/aifor_book_main_zh/main_zh.pdf`，页数 `507`。
...
- `python scripts/check_publication_blockers.py`：信息模式通过，并报告当前 `7` 个许可证、归属、引用、bibliography 和项目级 AI-use 未决项。
...
- `bash scripts/build_book_local.sh zh`：通过（当前中文书稿 `507` 页）
- `git diff --check`：通过
...
- 已完成 Part I/II 从头读者 QA，并已推送项目仓库 `a76629d`。最近一次完整验证包括：`bash scripts/build_book_local.sh zh` 通过；notebook smoke test `59` 个通过；data manifest `52` 个通过；`git diff --check` 通过。本轮 Part III--V 文本 QA 后已重新运行中文构建和 `git diff --check`，中文 PDF 当前为 `507` 页。
...
- 已新增 `scripts/check_publication_blockers.py`，默认以信息模式汇总许可证、归属、引用、bibliography 和项目级 AI-use 未决项；当前报告 `7` 个开放项，`--strict` 只应在发布决策完成后用于最终 gate。
