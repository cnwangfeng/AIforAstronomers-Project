...
在最近一次远端推送之后，完整项目仓库已经形成一组本地 checkpoint。当前 `main` 分支相对 `origin/main` 至少领先 `30` 个提交，具体数量以 `git status -sb` 为准；如继续写作，可以直接在当前状态上工作，如需要发布或备份，应先推送完整项目仓库，再同步 Overleaf 书稿仓库。
...
- Part II 最新补强已加入共同公式地图，并加厚距离模数推导、孔径测光净信号推导、红移相对变化、等效宽度离散求和、周期搜索线性系数到振幅/相位、固定 \(\tau\) 求最优 \(V_0\) 和 Monte Carlo 参数报告的解释；最新细读又为 HR 图、孔径测光和光谱结果补入最小结果报告模板。
...
- `ch11`、`ch12`、`ch13` 已进一步补入最小结果报告模板，把 HR 图、背景修正孔径测光、红移/EW 结果从“算出数值”推进到“说明数据选择、公式来源、诊断依据和适用边界”。
...
1. 确认是否推送完整项目仓库当前本地提交；当前 `main` 相对 `origin/main` 至少领先 `30` 个提交，具体数量以 `git status -sb` 为准。
...
5. 当前可不考虑 RPM；若后续用户重新提醒 RPM 约束，再恢复对应限制。本地编译、grep、git、notebook smoke test 不受网络/API RPM 约束。
