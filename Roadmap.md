...
- 完整项目仓库：`a76629d`，提交信息 `Complete Part I II reader QA pass`
- Overleaf 书稿仓库：`4da027b`，提交信息 `Sync textbook refresh and capstone chapters`

`a76629d` 已经把此前积累的 Part I/II 教材化细读、证据卡接口、release validation metadata 和路线图整理推送到 `origin/main`。如继续写作，可以直接在当前状态上工作；若本节之后又形成新的本地 checkpoint，具体领先数量以 `git status -sb` 和 `git log --oneline --decorate -5` 为准。

推送后当前继续的本地工作重点是 Stage B：Part III--V 的读者质量抽查已完成一轮 evidence-card 桥接；Part VI 已复查导读、收束和全书收束层，确认当前版本应保留三层交付模型和五个共同质量门，不再新增运维章节。

Overleaf 书稿仓库当前仍停在 `4da027b`。同步方式见本节后面的“书稿同步方式”；除非明确准备同步发布，否则不要直接运行 `sync_book_to_overleaf.sh --apply`。
...
- Part II 最新补强已加入共同公式地图，并加厚距离模数推导、孔径测光净信号推导、红移相对变化、等效宽度离散求和、周期搜索线性系数到振幅/相位、固定 \(\tau\) 求最优 \(V_0\) 和 Monte Carlo 参数报告的解释；最新细读已为 HR 图、孔径测光、光谱、周期搜索和实验拟合结果补入最小结果报告模板，并修正 `ch15` 一维参数搜索代码示例，使固定 \(\tau\) 后求最优 \(V_0\) 的推导与代码闭合。
- Part VI 导读已补入学生项目层、课程运行层和课程包维护层的三层阅读地图。
- Part VI 最新收束已补入共同质量门与 route 词汇，把 scope、evidence、boundary、owner 和 closure 统一成贯穿学生项目、评分、发布、交接和维护的共同判断语言；最新读者 QA 确认 Part VI 后续只需图表/引用/发布层检查，不再扩章节。
...
- `Part I/II` 已具备完整教材骨架，并已完成一轮从头读者细读：概念展开、公式/算法桥接、notebook 互引、常见错误、AI 助手边界、练习、小结和结果/运行记录模板均已接入。
- `Part III/IV/V` 的正文主线已经成体系；当前版本不再扩新方法章、新案例章或更深模型，只做读者质量、术语、图表和交叉引用修订。
- `Part VI` 已形成完整 capstone/课程包闭环；当前版本不再新增运维章节，导读与收束层已经把重复 checklist 压回三层交付模型、五个质量门和共同 route 语言。
...
- `bash scripts/build_book_local.sh main`：通过，兼容入口 PDF 输出为 `/tmp/aifor_book_main/main.pdf`。
- `bash scripts/build_book_local.sh zh`：通过，中文 PDF 输出为 `/tmp/aifor_book_main_zh/main_zh.pdf`，页数 `509`。
...
- `python scripts/check_latex_log.py`：通过，默认扫描 `/tmp/aifor_book_main_zh/main_zh.log`。
- `bash scripts/run_release_checks.sh quick`：通过，包含 data manifest、release inventory、publication blocker 信息报告、中文 LaTeX log、`git diff --check` 和 Overleaf dry-run payload 检查；当前 dry-run payload 为 `104` 个文件。
- `bash scripts/run_release_checks.sh full`：通过，完整复跑 data manifest、release inventory、publication blocker 信息报告、`59` 个 notebook smoke test、中英文 LaTeX 构建、中文 LaTeX log、`git diff --check` 和 Overleaf dry-run payload 检查。
- `git diff --check`：通过。
...
截至 2026-05-04，项目已经从“路线图”推进到“可验证的中文教材与课程包候选版本”，并完成以下内容：
...
- 后续不再大面积补写 Part I；只做逐章细读、术语统一、交叉引用、图表说明和读者反馈修订。
...
- `bash scripts/build_book_local.sh zh`：通过（当前中文书稿 `509` 页）
- `git diff --check`：通过
...
本节保留项目启动时的历史背景，用于理解仓库演化，不再作为当前待办清单。启动时仓库已经具备科学计算入门教材雏形：
...
- `main.tex` include、章节拆分、中文主入口和六大 Part 分隔已经重构完成。
...
- Python for Astronomers 及其他外部参照的授权、署名、改写和引用边界仍需做最终核查。
...
- Milestone 6 Capstone 与课程包：`ch39` 到 `ch59` 已形成学生项目、评分、试教、发布、交接、维护、应急和外部合作边界的课程包闭环。

当前仍开放的发布前任务：

- Reader QA：从头细读 Part I/II，然后抽查 Part III--VI，重点是教材口吻、术语一致、交叉引用、图表解释和练习层次。
- Figure QA：盘点全书图表和表格，确保每张图有正文引用、标题、单位、来源和解释边界。
- Source QA：核查参考文献、数据来源、许可证、AI 使用声明和 Python for Astronomers 等外部参照边界。
- Release QA：重新跑中文构建、notebook smoke test、data manifest、`git diff --check` 和 Overleaf dry-run；确认无阻塞后再推送项目仓库和书稿仓库。
...
根据 2026-05-04 的实际完成状态，后续工作按“收束优先、发布优先、只按反馈扩写”的原则执行。

阶段 A：关闭当前 Part II 报告模板补强

- 已完成 `ch14` 周期搜索结果报告模板和 `ch15` 实验拟合结果报告模板补强。
- 已完成 Part I/II 从头读者 QA，并已推送项目仓库 `a76629d`。最近一次完整验证包括：`bash scripts/build_book_local.sh zh` 通过；notebook smoke test `59` 个通过；data manifest `52` 个通过；`git diff --check` 通过。本轮 Part III--V 文本 QA 后已重新运行中文构建和 `git diff --check`，中文 PDF 当前为 `509` 页。
...
- Part VI 已完成本轮结构复查：导读、收束和全书收束层已经足够压住重复章节；后续不新增运维章节，只在 Stage C/E 做图表、引用、QA 和发布材料整理。
...
- 已建立 `book/FORMULA_QA.md`，记录当前中文主线 `140` 个 display math block、无编号 equation / align 环境，以及公式解释检查标准。
...
- 本轮已在完整项目仓库运行 `bash scripts/run_release_checks.sh full`，完整复跑 `python scripts/smoke_test_notebooks.py`、`python scripts/validate_data_manifest.py`、`python scripts/check_release_inventory.py`、`python scripts/check_publication_blockers.py`、`bash scripts/build_book_local.sh main`、`bash scripts/build_book_local.sh zh`、`python scripts/check_latex_log.py`、`git diff --check` 和 Overleaf dry-run payload guard；当前结果通过。
