...
- Part II 最新补强已加入共同公式地图，并加厚距离模数推导、孔径测光净信号推导、红移相对变化、等效宽度离散求和、周期搜索线性系数到振幅/相位、固定 \(\tau\) 求最优 \(V_0\) 和 Monte Carlo 参数报告的解释。
...
- `bash scripts/build_book_local.sh zh`：通过，中文 PDF 输出为 `/tmp/aifor_book_main_zh/main_zh.pdf`，页数 `487`。
...
- `bash scripts/build_book_local.sh zh`：通过（当前中文书稿 `487` 页）
- `git diff --check`：通过
