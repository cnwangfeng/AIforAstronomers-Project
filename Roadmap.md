...
- Part II 最新补强已加入共同公式地图，并加厚距离模数推导、孔径测光净信号推导、红移相对变化、等效宽度离散求和、周期搜索线性系数到振幅/相位、固定 \(\tau\) 求最优 \(V_0\) 和 Monte Carlo 参数报告的解释；最新细读已为 HR 图、孔径测光、光谱、周期搜索和实验拟合结果补入最小结果报告模板。
...
- `Part I/II` 已具备完整教材骨架：概念展开、公式/算法桥接、notebook 互引、常见错误、AI 助手边界、练习、小结和结果报告模板均已接入。
...
- `Part I` 读者细读已开始补齐最小记录模板：命令行运行记录、脚本运行记录、数据读入检查、版本关联结果记录和图表说明模板会共同进入最终 evidence cards。
...
- 先读 Part I/II：检查是否仍有“讲义式 list”、术语跳跃、公式缺少引入、notebook 互引不清或练习层次不稳的问题。
- 再抽查 Part III--V：重点看报告模板、baseline-first 语言、evaluation / interpretation / reproducibility 口径是否一致。
