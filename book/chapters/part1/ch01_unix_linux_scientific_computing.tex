...
\begin{itemize}
    ...
    \item 理解 \texttt{which}、\texttt{type}、\texttt{PATH}、环境变量和 alias 如何影响命令执行；
    \item 使用 \texttt{sort}、\texttt{uniq}、\texttt{cut} 和 \texttt{xargs} 对文本输出和路径列表做最小统计与批处理；
...
\section{Shell 环境：命令从哪里来}

当你输入 \texttt{python}、\texttt{ls} 或 \texttt{git} 时，shell 并不是凭空知道这些名字的含义。它会按照一组规则寻找要执行的程序，有些命令来自系统目录，有些来自你安装的环境，有些甚至是 shell 自己内置的功能。

这件事在科研计算里非常重要。许多“代码昨天能跑、今天不能跑”的问题，表面上像是 Python 或数据出错，实际原因却是你运行了另一个环境里的 \texttt{python}，或者脚本在远程服务器上找不到本地机器里才有的命令。

\subsection{\texttt{which}、\texttt{type} 与 \texttt{PATH}}

\texttt{which} 用来询问某个外部命令实际来自哪里；\texttt{type} 还能告诉你一个名字是不是 shell 内置命令、alias 或外部程序。
...
\begin{defbox}[\texttt{PATH}]
\texttt{PATH} 是 shell 用来查找可执行程序的一组目录。你输入一个命令名时，shell 会按 \texttt{PATH} 中的目录顺序寻找对应程序。
\end{defbox}

这解释了一个常见现象：同样输入 \texttt{python}，不同电脑、不同 conda 环境、不同远程服务器上运行的可能不是同一个解释器。因此，在课程项目和科研项目中，\texttt{which python}、\texttt{python --version} 和环境文件不是形式主义，而是在确认“我究竟用哪个工具生成了这个结果”。

\subsection{环境变量与 alias：方便，但要有边界}

环境变量可以把某些路径、选项或配置暴露给程序。例如：

\begin{examplebox}[设置一个项目路径变量]
\begin{lstlisting}[style=pythonstyle]
$ export PROJECT_ROOT=$HOME/gaia_project
$ echo $PROJECT_ROOT
\end{lstlisting}
\end{examplebox}
...
这些工具能提高交互效率，但不能滥用。特别是在脚本中，不要假设别人的终端里也定义了同样的 alias；需要依赖的环境变量也应在 README、运行脚本或课程说明中写清楚。否则，一个只在你自己电脑上“刚好能跑”的流程，换到同学、助教或服务器那里就会失效。
...
\subsection{shell 展开：命令真正收到的参数是什么}

当你输入一条命令时，程序通常不是直接看到你写下的原始文本。shell 会先做若干预处理，例如展开通配符、处理引号、识别重定向，再把一组参数交给真正的程序。

\begin{examplebox}[先预览 shell 展开结果]
\begin{lstlisting}[style=pythonstyle]
$ printf "%s\n" raw/*.fits
$ ls raw/*.fits
\end{lstlisting}
...
如果 \texttt{raw/} 里有三个 FITS 文件，那么 \texttt{rm raw/*.fits} 里的 \texttt{*.fits} 通常会先被展开成三个具体路径，然后 \texttt{rm} 收到的已经不是“一个模式”，而是一串文件名。理解这一点非常重要，因为它解释了两个常见现象：

\begin{itemize}
    \item 通配符匹配到多少文件，批量命令就会影响多少文件；
    \item 引号会改变 shell 的预处理方式，例如 \texttt{"*.fits"} 不再按通配符展开，而是作为普通字符串传给程序。
\end{itemize}

所以，批量操作前的预览并不是形式主义。你不是在“多敲一条命令”，而是在确认 shell 将要交给程序的真实目标集合。
...
\subsection{\texttt{sort}、\texttt{uniq} 和 \texttt{cut}：终端里的小型检查表}

许多科研检查并不需要立刻打开完整 notebook。只要数据是简单文本、日志或规则分隔的表格，终端就可以快速给出第一轮答案。

\begin{examplebox}[统计哪些日志文件包含错误信息]
\begin{lstlisting}[style=pythonstyle]
$ rg "ERROR" logs/*.txt | cut -d: -f1 | sort | uniq -c
\end{lstlisting}
\end{examplebox}
...
\begin{enumerate}
    \item \texttt{rg} 找出包含 \texttt{ERROR} 的日志行；
    \item \texttt{cut -d: -f1} 取出冒号前的文件名；
    \item \texttt{sort} 把相同文件名排在一起；
    \item \texttt{uniq -c} 统计每个文件出现了多少次。
\end{enumerate}

如果你只想快速查看一个逗号分隔小表格中的某一列，也可以先用 \texttt{cut} 做粗略检查：

\begin{examplebox}[粗略查看某一列的取值分布]
\begin{lstlisting}[style=pythonstyle]
$ cut -d, -f2 raw/catalog.csv | sort | uniq -c
\end{lstlisting}
\end{examplebox}

\begin{warnbox}[文本工具不是通用 CSV 解析器]
\texttt{cut} 很适合处理简单分隔文本，但它不理解带引号、嵌入逗号或复杂缺失值规则的 CSV。真正进入科学分析时，仍应使用 Python 的 \texttt{csv} 模块、Pandas 或 Astropy Table 来可靠解析表格。这里的终端工具主要用于快速预览和 sanity check。
\end{warnbox}

\subsection{\texttt{xargs}：把路径列表交给下一条命令}

\texttt{find} 和 \texttt{rg} 经常生成一串候选路径。下一步常常不是“看完这些路径”，而是把这些路径交给另一个命令继续处理。此时可以使用 \texttt{xargs}。

\begin{examplebox}[统计一批日志文件的行数]
\begin{lstlisting}[style=pythonstyle]
$ find logs -name "*.txt" -print0 | xargs -0 wc -l
\end{lstlisting}
\end{examplebox}

这里的 \texttt{-print0} 和 \texttt{xargs -0} 表示用空字符分隔路径，而不是用空格或换行分隔。这种写法看起来稍微复杂，但它能更安全地处理带空格的文件名。入门阶段可以先记住这条原则：\textbf{只要路径是批量生成的，就要格外关心它们如何被传给下一条命令。}
...
\begin{warnbox}[重定向也是一种写操作]
\texttt{>} 会覆盖已有文件。如果你运行 \texttt{python analyze.py > results.txt}，而 \texttt{results.txt} 原来已经保存了重要输出，那么原内容会被新输出替换。需要保留历史时，应考虑使用 \texttt{>>} 追加、换一个带日期或版本的文件名，或者先把旧文件归档。
\end{warnbox}
...
\subsection{用 \texttt{tee} 同时看输出和保存记录}

有些时候你既想在屏幕上看到程序输出，又想把它保存成日志。直接用 \texttt{>} 会让屏幕上不再显示正常输出；这时可以使用 \texttt{tee}。

\begin{examplebox}[同时显示和保存]
\begin{lstlisting}[style=pythonstyle]
$ python analyze.py | tee logs/analyze_run.txt
\end{lstlisting}
\end{examplebox}

\texttt{tee} 的名字来自管道里的 T 形分流：一份输出继续显示在终端，一份写入文件。对课程项目来说，这是一种很好的运行证据：以后你不仅有结果文件，还能看到当时程序报告了哪些输入、参数和摘要统计。
...
\subsection{长任务的最小运行记录}

长时间任务最怕的不是慢，而是慢完之后没有证据。至少应记录三类信息：
...
\begin{examplebox}[给长任务留下证据]
\begin{lstlisting}[style=pythonstyle]
$ time python reduce_night.py > logs/reduce.out 2> logs/reduce.err
\end{lstlisting}
\end{examplebox}

\texttt{time} 会报告程序运行耗时，\texttt{logs/reduce.out} 保存正常输出，\texttt{logs/reduce.err} 保存错误信息。这个例子还不等于完整的集群作业系统，但已经具备了科研工作流最重要的意识：不要只相信“我按过回车”，而要留下“它如何运行、是否失败、用了多久”的记录。
...
这里的 \texttt{set -euo pipefail} 是一个常见的安全开关组合：

\begin{itemize}
    \item \texttt{-e}：某条命令失败时尽早停止脚本；
    \item \texttt{-u}：使用未定义变量时报错；
    \item \texttt{pipefail}：管道中间步骤失败时，不要只看最后一条命令是否成功。
\end{itemize}

入门阶段不必把它当成魔法咒语背下来，但要理解它的意图：科研脚本应该尽早暴露失败，而不是悄悄带着错误继续生成貌似正常的结果。
...
\begin{enumerate}
    \item 建立项目目录结构；
    \item 确认 shell 环境和关键命令来自预期位置；
    \item 把原始数据与处理结果分开；
    \item 用明确命名约定组织文件；
    \item 每次批量操作前先列出目标文件；
    \item 用搜索、排序、计数和路径传递命令做一致性检查；
...
Linux 命令行强大的另一面，是它很少主动替你判断“这个操作是不是危险”。尤其是 \texttt{rm}、\texttt{mv}、覆盖重定向和批量通配符，一旦目标选错，后果可能比图形界面里的误删更难恢复。

因此，任何破坏性操作前都建议先做三步确认：

\begin{enumerate}
    \item \textbf{确认当前位置}：先运行 \texttt{pwd}，确保自己在预期项目目录中；
    \item \textbf{预览目标集合}：先用 \texttt{ls}、\texttt{find} 或 \texttt{rg} 列出将被影响的文件；
    \item \textbf{先做小范围试运行}：能处理一个文件时，不要一开始就处理一千个文件。
\end{enumerate}

\begin{warnbox}[先列出，再删除]
如果你准备运行类似 \texttt{rm *.fits} 的命令，请先运行 \texttt{ls *.fits} 看看通配符到底匹配了哪些文件。命令行里最常见的灾难，往往不是不会用命令，而是太相信自己已经选对了目标。
\end{warnbox}

这个习惯看起来保守，但它是科研数据管理里非常实际的安全边界。原始数据、手工整理结果和长时间计算输出，都应该先被确认、归档或版本记录，再进入批量删除或移动步骤。

\section{一个最小命令行运行记录模板}

本章讲了很多命令，但真正需要进入科研记录的不是每一次 \texttt{ls}，而是那些会改变项目状态、生成结果、移动数据或影响后续复现的操作。一个最小命令行运行记录可以写成：

\begin{quote}
在项目根目录运行准备脚本，脚本位于 \nolinkurl{scripts/prepare_demo_tree.sh}。输入来自 \nolinkurl{raw/}，输出写入 \nolinkurl{results/} 与 \nolinkurl{logs/}。运行前用 \texttt{pwd} 和 \texttt{find} 确认目录与文件集合；日志保存在 \nolinkurl{logs/prepare.log}；未修改原始数据。
\end{quote}

这段记录包含四类信息：当前位置、输入集合、执行入口和输出位置。它不追求复杂，却能回答未来最常见的复查问题：当时在哪个目录运行？处理了哪些文件？结果写到哪里？有没有可能覆盖或移动原始数据？到了最终项目包，这类记录会成为 Reproducibility card 和 Integrity card 的基础材料。

\section{配套 notebook 做了什么}

配套 notebook 不要求学生真的连接远程集群，也不依赖本机已有的大型数据目录。它会临时创建一个小型科研项目目录，并带你完成以下动作：

\begin{itemize}
    \item 建立 \texttt{raw/}、\texttt{intermediate/}、\texttt{logs/}、\texttt{scripts/} 和 \texttt{results/} 目录；
    \item 练习 \texttt{pwd}、\texttt{ls}、相对路径和绝对路径；
    \item 用 \texttt{find} 与 \texttt{rg} 搜索文件名和日志内容；
    \item 检查目录大小、磁盘空间和脚本执行权限；
    \item 把结果目录打包成一个最小归档。
\end{itemize}

它的目标不是把所有 Linux 命令都演示一遍，而是让你第一次体验“命令行如何把项目结构、搜索检查、权限和归档连成一条工作流”。
...
\begin{itemize}
    ...
    \item 不确认 \texttt{python}、\texttt{pip} 或 \texttt{git} 实际来自哪个环境；
    \item 把自己终端里的 alias 当成所有人都能运行的脚本依赖；
    \item 文件名里有空格或特殊字符，却没有用引号保护；
    \item 在不知道当前位置时执行 \texttt{rm} 或 \texttt{mv}；
    \item 用普通空格分隔的 \texttt{xargs} 处理复杂文件名，导致路径被错误拆开；
