...
这和本书后面的 capstone evidence cards 是同一思想：结果不是孤立文件，而是一组可复查证据。
...
本章的特殊通过条件是：必须能指出哪个 commit 或 tag 生成了哪个输出文件。这份记录会在后续章节继续升级，最终成为 capstone reproducibility card 的一部分。
...
这条算法会在后面的书稿发布、notebook smoke test、数据 manifest 和 capstone package 中反复出现。Git 的最终目标不是让历史变复杂，而是让阶段性结果可以被可靠引用。
