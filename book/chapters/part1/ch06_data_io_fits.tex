...
本章需要精读并动手。目标不是学完所有文件格式，而是建立数据入口契约：一个文件被读入之前，必须知道来源、格式、字段或 header、单位、缺失值、质量标记和后续用途。

最低交付是一个 Evidence Record 和一份数据入口检查输出。本章进入 Part III 的作用，是保护特征、标签、质量标记和预处理历史不在读文件的第一步就丢失。
...
训练目标是完成一个小型数据入口任务：读入一份星表 CSV 和一个教学版 FITS 图像，做字段/header 检查、缺失值检查、最小统计和背景修正示例，最后交付一份数据入口 Evidence Record。
...
\begin{enumerate}
    ...
    \item 在数据入口 Evidence Record 中写明原始输入、清洗输出和脚本入口。
\end{enumerate}
...
任何会改变样本集合的操作都应留下记录。尤其是删除缺失值、剔除异常值、按质量标记筛选、单位转换和重命名字段，都应该能在脚本或数据入口 Evidence Record 中找到理由。
\end{warnbox}
...
\begin{enumerate}
    ...
    \item 写入数据入口 Evidence Record，并保留原始输入的追溯线索。
\end{enumerate}
...
\section{把入口检查写成 Evidence Record}

本章最重要的交付物不是一段漂亮代码，而是一份可复查的数据入口 Evidence Record。它回答的不是“文件能不能打开”，而是“这个文件能否被后续分析正确解释”。
...
这份记录不应变成独立模板，而应作为统一 Evidence Record 的本章附加字段：

\begin{examplebox}[Ch06 Evidence Record 附加字段]
\begin{lstlisting}[style=terminal]
Ch06 Fields:
- file path:
- data source:
...
本章的特殊通过条件是：必须说明字段或 header、单位、缺失值或质量标记；不能只说“文件可以打开”。这份记录会进入后续 evidence cards 中的 Data 和 Reproducibility 部分。它让别人知道你不是“随手读了一个文件”，而是做了入口质量控制。
...
\begin{enumerate}
    \item 数据文件来源和路径是否清楚？
    \item 字段/header 和单位是否说明？
    \item 缺失值或质量标记是否记录？
    \item 哪些列/数组会进入后续分析是否明确？
\end{enumerate}

最低交付：Evidence Record 和数据入口检查输出。

\section{本章小结}

数据读取不是把文件塞进程序，而是建立数据和科学解释之间的第一座桥。一个可靠的数据入口流程必须检查路径、格式、字段、类型、单位、缺失值、header、数组形状和来源记录。只要这一步做扎实，后续绘图、统计、机器学习和 后续项目才有可信输入。忽略 I/O 质量控制，后面的模型越复杂，错误越难发现。
