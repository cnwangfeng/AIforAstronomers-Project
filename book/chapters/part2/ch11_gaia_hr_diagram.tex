...
最低交付是一个 Evidence Record、一张 HR 图和一个 Data Card 草稿。本章进入 Part III 的作用，是示范如何把原始星表列转成可解释特征，并记录质量筛选、选择效应和物理解释边界。

\section{本章导读：从星表行列到恒星物理图像}

本章表面上是在画一张 HR 图，真正训练的却是另一件事：怎样把一张星表中的列，变成有物理含义的派生量，再把这些派生量组织成一幅可以解释恒星结构的图。星表分析不是“把 CSV 读进来再散点图一下”，而是要先判断每一列代表什么观测量、单位是什么、质量是否足够，以及哪些列可以通过公式组合成新的物理量。

HR 图特别适合作为第一类天文表格项目，因为它把几个核心动作压缩在一起：用视差估计距离，用距离模数把表观星等变成绝对星等，用颜色指数近似追踪温度，再用坐标轴方向、筛选条件和样本边界解释主序、巨星和白矮星结构。任何一步说不清楚，图都可能只是“看起来像 HR 图”，而不是一个可信的物理证据。

读这一章时，请始终追问两个问题：第一，图上的每个点从哪几列原始数据来；第二，图上看到的结构有多少来自恒星物理本身，又有多少可能来自样本选择、视差质量和可视化设置。这两个问题会直接进入后面的特征工程和机器学习案例。
...
本章的微型项目是把一个 Gaia 风格教学星表转化成一张最小 HR 图。正文会解释星表字段、距离模数、绝对星等、颜色指数和选择效应；配套训练则要求你把这些概念落到一个可复跑的小项目中。

\begin{examplebox}[最小 HR 图项目结构]
\begin{lstlisting}[style=terminal]
hr_diagram_demo/
  data/
    stellar_photometry_demo.csv
  notebooks/
    build_hr_diagram.ipynb
  scripts/
    make_hr_diagram.py
  figures/
    hr_diagram_demo.png
...
最终交付不只是图片，而是一条完整证据链：字段契约、筛选条件、绝对星等公式、图像文件、图注、物理解释和限制说明。后续机器学习章节会把类似的派生列当作特征，因此本章也在训练你记录“特征从哪里来”。
...
这个公式不应只被当成记忆结论。星等系统本身来自亮度比：

\[
m_1 - m_2 = -2.5\log_{10}\left(\frac{F_1}{F_2}\right),
\]

其中 $F$ 是接收到的辐射通量。若同一颗恒星分别放在距离 $d$ 和标准距离 $10\,\mathrm{pc}$ 处，按照平方反比定律，

\[
\frac{F(d)}{F(10\,\mathrm{pc})}
= \left(\frac{10\,\mathrm{pc}}{d}\right)^2.
\]
...
\[
m - M
= -2.5\log_{10}\left[\left(\frac{10}{d}\right)^2\right]
= 5\log_{10}\left(\frac{d}{10}\right).
\]

这一步把 HR 图里最重要的思想说清楚了：绝对星等不是重新定义一个新字段，而是把“看起来有多亮”校正到同一个标准距离后，再比较恒星本征亮度。没有这一步，颜色--星等图会把本征物理差异和距离差异混在一起。
...
在 HR 图任务中，\texttt{absolute\_g\_mag} 看起来只是星表里新增的一列，但它和原始观测列有本质差别。它依赖 \texttt{g\_mag}、\texttt{parallax\_mas}、单位约定和距离反演近似。因此，一个派生列至少应该带着三类说明：

\begin{itemize}
    \item 它由哪些原始列计算得到；
    \item 使用了哪条公式，以及公式的单位约定是什么；
    \item 在哪些样本上这条公式可能不稳定，例如低信噪比视差或负视差。
\end{itemize}

这也是后面机器学习章节会反复强调的“特征来源”问题。模型看到的只是一个数值列，但科学解释必须知道这个数值列背后经历过怎样的物理变换和质量筛选。
...
\section{一个最小 HR 图结果报告模板}

完成 HR 图以后，结果报告不能只写“图中可以看见主序星”。更好的写法应把数据选择、派生量公式、图形结构和解释边界连在一起。例如：

\begin{quote}
本图使用满足正视差和有限颜色指数条件的教学星表样本，横轴为 \texttt{BP-RP} 颜色指数，纵轴为由 \(M_G = g-10+5\log_{10}p(\mathrm{mas})\) 得到的绝对 \(G\) 星等，并采用反向纵轴。样本在颜色--绝对星等平面上形成从蓝亮到红暗的主带，同时有少量位于亮端或蓝暗端的对象。当前解释只适用于这个小型教学样本；真实 Gaia 数据还需要进一步检查视差信噪比、消光、双星和选择效应。
\end{quote}

这段话包含四个层次：第一，说明哪些对象进入了图；第二，说明纵轴不是原始观测列，而是由公式派生出的物理量；第三，把图上的结构翻译成恒星物理语言；第四，主动声明当前结论的适用边界。对后面的机器学习任务来说，这种写法也能帮助你区分“原始字段”“派生特征”和“人工解释标签”。

\section{本章 Evidence Record 附加字段}

本章不另建独立 HR 图记录模板，而是使用统一 Evidence Record，并在最后补充少量星表和 HR 图字段：

\begin{examplebox}[Ch11 Evidence Record 附加字段]
\begin{lstlisting}[style=terminal]
Ch11 Fields:
- catalog/table path:
- source columns:
...
本章的特殊通过条件是：必须说明筛选条件和选择效应；不能只给 HR 图。
...
\begin{enumerate}
    \item 星表字段、单位和质量筛选是否清楚？
    \item 派生量公式是否可复查？
    \item HR 图解释是否区分 claim 和 limit？
    \item 可进入 ML 的特征和风险是否说明？
\end{enumerate}

最低交付：Evidence Record、HR 图和 Data Card 草稿。
