...
最低交付是一个 Evidence Record 和图像检查结果或 photometry/cutout 结果。本章进入 Part III 的作用，是记录 cutout 大小、背景处理、归一化、mask、像素尺度和标签来源，避免图像数组脱离科学语境。

\section{本章导读：像素为什么还不是天文测量}

图像是很多学生最容易产生直觉的数据类型：把二维数组画出来，好像就已经“看见了天体”。但在科研分析里，像素图只是起点。你真正想报告的通常不是某个像素值，而是源的位置、亮度、背景修正后的净信号，或者它与另一份星表、光谱和模型之间的对应关系。

这就要求我们把图像拆成一条证据链：数组记录探测器响应，header 记录观测和坐标条件，WCS 把像素位置连接到天空坐标，背景估计把源信号和天空背景分开，孔径测光公式再把一片像素区域压缩成一个带单位和限制的测量值。缺少其中任何一环，“这颗源有多亮”都很难被认真回答。

本章使用教学 cutout 而不急着处理大型 FITS 文件，是有意为之。目标不是先学会所有真实文件细节，而是先掌握图像分析的最小结构：像素、元数据、坐标、背景、孔径和结果记录如何互相支撑。
...
进入本章前，建议你已经能读懂基础文件 I/O、循环、字典和小函数，因为本章会把这些能力用在图像数组、header 记录和坐标变换的最小流程里。

\section{训练目标与贯穿微型项目}

本章的微型项目是一个教学 cutout 测光任务。正文先帮助你看懂图像数组、FITS header、WCS 和背景修正孔径测光的关系；训练材料则要求你完成一次可复查的图像测量，而不是只把图显示出来。

\begin{examplebox}[最小图像测光项目结构]
\begin{lstlisting}[style=terminal]
image_cutout_demo/
  data/
    fits_wcs_cutout_demo.csv
  notebooks/
    aperture_photometry.ipynb
  scripts/
    measure_cutout_flux.py
  figures/
    cutout_aperture.png
  results/
    image_evidence_record.txt
\end{lstlisting}
\end{examplebox}

项目交付应包含：cutout 数据入口、教学版 header、背景估计方法、孔径定义、净信号公式、近似天空坐标和测光结果说明。这样做的目的，是让你在进入真实 FITS 文件前先形成一个关键直觉：图像值、坐标和物理量之间必须通过元数据和算法连接起来。
...
\begin{warnbox}[像素坐标约定必须先说清楚]
真实 FITS/WCS 文档、Python 数组索引和绘图坐标有时采用不同的起点约定。Python 数组索引通常从 $0$ 开始，而 FITS 关键词里的参考像素常按天文约定书写。教学代码可以采用统一的简化约定，但真实项目中必须明确：当前公式里的 $x,y$ 到底是数组索引、像素中心坐标，还是 FITS/WCS 坐标。
\end{warnbox}
...
其中 $Q_i$ 是目标源贡献，$B_i$ 是背景贡献。孔径内直接相加得到的是

\[
S_{\mathrm{ap}} = \sum_{i\in \mathrm{ap}} C_i
= \sum_{i\in \mathrm{ap}} Q_i + \sum_{i\in \mathrm{ap}} B_i.
\]

如果背景在局部区域内可以近似为常数，且背景平均值估计为 $B$，那么孔径内背景总贡献可以近似为 $N_{\mathrm{pix}}B$。于是目标源的净信号估计就是

\[
\hat{Q} = S_{\mathrm{ap}} - N_{\mathrm{pix}}B.
\]

这正是前面 $S_{\mathrm{net}}$ 公式的来源。这个推导虽然简单，却很重要：它说明背景扣除不是一个图像美化步骤，而是把“源信号”和“观测环境贡献”分开的物理估计。

\subsection{净信号的不确定度从哪里来}

背景修正后的 $S_{\mathrm{net}}$ 也不是一个绝对精确的数。即使孔径位置完全正确，净信号的不确定度至少会受到两类波动影响：
...
若孔径内每个像素的方差为 $\sigma_i^2$，背景区域有 $N_B$ 个像素，背景像素样本方差为 $s_B^2$，一个最小近似可以写成：

\[
\sigma^2(S_{\mathrm{net}})
\approx
\sum_{i\in \mathrm{ap}}\sigma_i^2
+ N_{\mathrm{pix}}^2\frac{s_B^2}{N_B}.
\]

第一项说明孔径内像素越多，孔径噪声会累积；第二项说明背景均值虽然是从背景区域估出来的，但它被乘以 $N_{\mathrm{pix}}$ 后会影响整个孔径修正。真实测光还会加入 Poisson 噪声、读出噪声、增益和零点误差等项，但这个最小公式已经足以说明：\textbf{背景不是扣掉就消失了，它的不确定度也会跟着进入结果。}
...
前面的背景修正和 WCS 线性近似，都可以直接写成最小代码。这样做的目的，不是把物理压缩成几行脚本，而是让学生看到公式、变量和计算步骤之间的对应关系。

\begin{examplebox}[背景修正与坐标映射的最小代码]
\begin{lstlisting}[style=pythonstyle]
import math

background_mean = sum(background_pixels) / len(background_pixels)
net_flux = aperture_sum - aperture_n_pix * background_mean

ra_deg = crval1 + (x_pix - crpix1) * cdelt1 / math.cos(math.radians(crval2))
dec_deg = crval2 + (y_pix - crpix2) * cdelt2
\end{lstlisting}
\end{examplebox}

\section{一个最小测光结果报告模板}

完成背景扣除和孔径求和以后，也不要只写“净信号是多少”。图像测光结果至少应交代孔径、背景、像素尺度和坐标假设。一个教学报告可以写成：

\begin{quote}
在 \(7\times7\) 教学 cutout 中，我们以中心源附近像素定义圆形孔径，用边缘像素估计局部平均背景，并按 \(S_{\mathrm{net}}=S_{\mathrm{ap}}-N_{\mathrm{pix}}B\) 得到背景修正净信号。该结果依赖当前孔径半径、背景区域选择和线性 WCS 近似；若用于真实图像，还需要检查曝光时间、增益、读出噪声、坏像素、邻近源污染和完整 FITS header。
\end{quote}

这个模板的价值在于把一个看似简单的数值拆回它的来源。孔径测光不是“把中心几个像素相加”，而是一次带有几何选择、背景估计和元数据假设的测量。后面如果你把测光结果作为机器学习特征，这些假设也应当随特征一起被记录。

\section{本章 Evidence Record 附加字段}

本章不另建独立测光记录模板，而是使用统一 Evidence Record，并补充图像、WCS 和 cutout 字段：

\begin{examplebox}[Ch12 Evidence Record 附加字段]
\begin{lstlisting}[style=terminal]
Ch12 Fields:
- FITS file:
- HDU/header checked:
...
本章的特殊通过条件是：必须说明背景、归一化、cutout 或 aperture 设置中的至少一项；不能只给 imshow 图。
...
\begin{enumerate}
    \item FITS header/data 和图像形状是否检查？
    \item 坐标、像素尺度或 WCS 风险是否说明？
    \item 背景、cutout、aperture 或归一化是否记录？
    \item 图像进入 ML 前会丢失什么信息是否说明？
\end{enumerate}

最低交付：Evidence Record 和图像检查结果或 photometry/cutout 结果。
