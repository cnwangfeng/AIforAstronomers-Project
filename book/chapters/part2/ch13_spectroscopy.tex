...
最低交付是一个 Evidence Record 和光谱图或红移/谱线记录。本章进入 Part III 的作用，是记录波长网格、归一化、重采样、mask、S/N 和标签来源，避免模型只看到数组而失去物理坐标。

\section{本章导读：沿着波长轴读出物理线索}

如果图像主要回答“天体在哪里、形态怎样”，光谱更进一步追问“它由什么组成、处在什么物理状态、相对我们如何运动”。一条光谱看起来只是波长和流量两列数，但波长轴上的峰、谷、宽度和位移，往往直接连接到原子跃迁、温度、速度、红移和仪器分辨率。

因此，光谱分析不能简化成“找到一个最高点或最低点”。在报告任何谱线之前，你需要知道波长单位和校准是否可信，连续谱如何处理，候选线心怎样估计，单线判断是否可能误认，多条谱线是否给出一致红移，以及信噪比是否足以支持当前解释。

本章的目标，是让你把光谱当成一条有物理秩序的一维数据，而不是普通表格。后续的光谱分类、异常检测和深度模型都可能吞进大量光谱数组；但如果这里没有先建立谱线、归一化、红移和等效宽度的解释边界，后面的模型输出就很难被翻译回可靠的物理判断。
...
进入本章前，建议你已经能处理小表读取、排序、分组、循环和函数封装，因为本章的核心步骤之一就是把长表重组为按对象聚合的光谱切片。

\section{训练目标与贯穿微型项目}

本章的微型项目是从一个极小光谱表中测量候选谱线并估计红移。正文负责解释波长轴、流量、连续谱、谱线、分辨率、归一化和红移公式；训练材料则要求你把这些概念整理成一条可复跑的光谱处理流程。

\begin{examplebox}[最小光谱项目结构]
\begin{lstlisting}[style=terminal]
spectrum_redshift_demo/
  data/
    spectral_wavelength_redshift_demo.csv
  notebooks/
    measure_redshift.ipynb
  scripts/
    measure_spectrum_features.py
  figures/
    spectrum_redshift_demo.png
  results/
    spectrum_evidence_record.txt
\end{lstlisting}
\end{examplebox}

最终交付应说明：波长单位、候选谱线身份、连续谱处理方式、线心估计方法、红移计算、速度近似是否适用，以及单条谱线判断的限制。后面光谱分类、异常检测和深度学习章节都会回到这些记录。
...
\subsection{为什么红移要写成相对变化}

红移公式的分母放在 $\lambda_{\mathrm{rest}}$，不是任意约定，而是为了把“移动了多少波长”写成一个无量纲的相对变化。若同样移动 $10\,\text{\AA}$，它对紫外短波线和红光长波线的意义并不完全一样；真正可比较的是

\[
\frac{\Delta \lambda}{\lambda_{\mathrm{rest}}}
= \frac{\lambda_{\mathrm{obs}}-\lambda_{\mathrm{rest}}}
{\lambda_{\mathrm{rest}}}.
\]

这和误差分析中的相对误差很像：绝对差值告诉你数值移动了多少，相对差值告诉你这个移动相对于原始尺度有多大。红移之所以能成为跨谱线、跨对象比较的参数，正是因为它使用的是这种相对波长变化。
...
\subsection{为什么离散求和里不能丢掉 \texorpdfstring{$\Delta\lambda$}{Delta lambda}}

等效宽度的单位是波长单位，例如 \(\text{\AA}\)。因此，离散求和里的 \(\Delta\lambda_i\) 不是装饰项，而是在把“每个采样点的相对凹陷或凸起”转换成“覆盖了多少波长范围的面积”。

如果所有光谱都在同一个均匀波长网格上，\(\Delta\lambda_i\) 可能只是一个常数，看起来不太显眼；但一旦采样间隔不同，直接把 \((1-F_i/C_i)\) 相加就会错误地把“采样点个数”当作“谱线面积”。这也是为什么波长排序、采样间隔和重采样会进入光谱特征测量，而不是只影响画图。
...
\begin{warnbox}[重采样不是无损操作]
重采样会改变相邻点之间的相关性，也可能平滑窄谱线或放大边界插值误差。因此，报告光谱特征时不应只写“已经重采样”，还应说明目标波长网格、插值方法和是否检查了谱线位置与线强的稳定性。
\end{warnbox}
...
\section{一个最小光谱结果报告模板}

光谱分析的报告也不能只给出一个 \(z\) 或一个 EW 数字。一个更稳妥的教学写法可以是：

\begin{quote}
对 demo 光谱，我们先按对象聚合并按波长排序，在局部连续谱归一化后识别出候选 H\(\alpha\) 发射结构。若采用 \(\lambda_{\mathrm{rest}}=6563\,\text{\AA}\) 并测得线心约为 \(6694\,\text{\AA}\)，则 \(z\approx0.020\)，小红移近似下对应速度量级约 \(6000\,\mathrm{km\,s^{-1}}\)。该判断需要进一步用多条谱线一致性、局部信噪比、采样间隔和归一化稳定性检查；单条谱线不能单独冻结最终红移。
\end{quote}

如果报告等效宽度，也应同时说明积分窗口、连续谱估计、符号约定和 \(\Delta\lambda\) 的处理方式。这样读者才能判断这个特征是否可以和其他对象比较，或者是否适合作为后续分类、聚类和异常检测的输入。

\section{本章 Evidence Record 附加字段}

本章不另建独立光谱记录模板，而是使用统一 Evidence Record，并补充光谱轴、谱线和红移字段：

\begin{examplebox}[Ch13 Evidence Record 附加字段]
\begin{lstlisting}[style=terminal]
Ch13 Fields:
- spectrum file:
...
本章的特殊通过条件是：必须说明红移或谱线判断的质量依据；不能只标出一个峰。
...
\begin{enumerate}
    \item 波长和流量单位是否清楚？
    \item 连续谱、归一化或重采样是否记录？
    \item 谱线/红移判断是否有质量依据？
    \item 光谱进入 ML 前的预处理风险是否说明？
\end{enumerate}

最低交付：Evidence Record 和光谱图或红移/谱线记录。
