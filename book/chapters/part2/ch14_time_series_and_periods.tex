...
最低交付是一个 Evidence Record 和 period search/phase-folding 结果。本章进入 Part III 的作用，是保留 cadence、duration、missing pattern、period feature、amplitude feature、label source 和 alias 风险。
...
这段报告有四个关键部件：第一，说明搜索范围，而不是把算法当作全宇宙搜索；第二，说明比较标准，例如残差、\(\chi^2\) 或周期图功率；第三，说明折叠图是否支持候选；第四，主动保留采样窗口和混叠风险。到了后续项目证据包，这些内容会分别进入 Data Card、Model Experiment Record、Trust Statement 和 Evidence Record。

\section{本章 Evidence Record 附加字段}

本章不另建独立周期记录模板，而是使用统一 Evidence Record，并补充时间序列和候选周期字段：

\begin{examplebox}[Ch14 Evidence Record 附加字段]
\begin{lstlisting}[style=terminal]
Ch14 Fields:
- time series file:
- time unit/system:
...
本章的特殊通过条件是：不能只报告 best period；必须说明 second-best period 或 alias 风险。
...
\begin{enumerate}
    \item 时间戳、时间跨度和采样是否说明？
    \item 周期搜索范围是否合理？
    \item 是否报告 best period 之外的候选或 alias 风险？
    \item 时间序列进入 ML 前的缺失/采样风险是否说明？
\end{enumerate}

最低交付：Evidence Record 和 period search/phase-folding 结果。
