...
physics_fit_demo/
  ...
  results/
    model_fit_record.txt
\end{lstlisting}
\end{examplebox}

项目交付应包含：变量单位、候选模型、参数搜索范围、最佳参数、残差诊断、Monte Carlo 不确定度、baseline 比较和模型假设限制。这个结构会为 Part III 的模型评估和 Part VI 的 capstone 结果报告打底。
...
例如，教学报告可以写成：“在原始电压空间中，用指数衰减模型拟合得到 $\tau \approx 2.15\,\mathrm{s}$；Monte Carlo 重采样给出的 $\tau$ 标准差约为若干百分之一秒；残差未显示强系统趋势，但该结论仍依赖当前误差条和理想 RC 模型假设。”这种写法比孤立参数更可信，因为它把数值、误差、诊断和适用边界放在了一起。到了最终项目包，原始实验字段和误差模型进入 Data card，拟合流程和随机扰动方法进入 Reproducibility card，残差与 baseline 比较进入 Evaluation card，物理解释和模型假设则进入 Interpretation card。
