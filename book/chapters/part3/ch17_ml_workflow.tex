...
write_model_record(evaluation, errors)
\end{lstlisting}
\end{examplebox}
...
本章最终希望你形成一份 \texttt{model\_record.txt}。它不需要很长，但至少应该包含：
...
\begin{examplebox}[模型记录的最小字段]
\begin{lstlisting}[style=pythonstyle]
model_record = {
    "task": "stellar class prediction",
    "split": "one held-out example per class",
...
这类记录会在后续章节不断变厚：回归会加入残差，分类会加入按类别指标，模型选择会加入验证曲线，可信度章节会加入覆盖范围。这里先把骨架立起来。
