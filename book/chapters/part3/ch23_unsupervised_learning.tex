...
本章的微型项目是在 HR 图特征上探索无监督结构。正文负责解释 PCA、\(k\)-means、DBSCAN、Gaussian mixture、t-SNE/UMAP 以及“算法结构不等于自然类别”；训练材料则要求你把无标签探索写成候选结构记录。
...
在 capstone 语境里，无监督报告通常不会直接变成最终物理结论，而是进入 Interpretation card 和后续人工复核队列。它的价值是提出“值得进一步检查的结构”，而不是替研究者完成类别命名。
