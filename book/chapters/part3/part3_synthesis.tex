...
Part III 的方法章节现在对应一组连续的模型记录：

\begin{enumerate}
    \item ch16 先形成 problem statement；
    \item ch17 形成 workflow record；
    \item ch18 形成 regression record；
    \item ch19 形成 classification record；
    \item ch20 形成 evaluation record；
    \item ch21 形成 preprocessing record；
    \item ch22 形成 model selection record；
    \item ch23 形成 unsupervised record；
    \item ch24 形成 anomaly review record；
    \item ch25 形成 trust statement。
\end{enumerate}

这些记录共同回答一个问题：模型结果从哪里来，为什么值得暂时相信，哪里必须降级为候选或限制说明。到了 Part IV，它们会成为每个案例项目的内部结构。
