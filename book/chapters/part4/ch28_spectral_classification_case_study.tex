...
\section{案例导读：高维数据需要先整理成可比较证据}

光谱分类很容易被误解成“每条光谱是一行高维特征，直接送进分类器”。但光谱不是普通高维表格。波长网格、归一化方式、坏像元、信噪比、连续谱形状和谱线位置，都会影响模型看到的结构，也会影响错分样本应该怎样解释。

这个案例把 Part II 的光谱理解和 Part III 的预处理、降维、分类、诊断重新装配起来。PCA 在这里不是为了炫技，而是帮助我们先看光谱样本的主要变化方向；分类器也不是终点，真正的终点是能回到错分光谱上判断问题来自数据质量、预处理、特征表示还是类别边界。

读这一章时，请特别留意“预处理”这个词。对光谱来说，预处理不是低级杂活，而是决定物理信息是否被保留下来的关键科学选择。
...
进入 Part IV case package 时，光谱输入契约和预处理记录应进入 Dataset Contract 与 Model Experiment Record；PCA 和最近中心分类器应作为 Part III Algorithm Card 的 sidebar links；PCA 平面、混淆矩阵和错分光谱复查进入 Evaluation artifact 与 Error analysis artifact；CNN 升级理由和当前教学数据边界则进入 Trust Statement。

本章配套 notebook 的最后一步会导出一份最小样板包：

\begin{examplebox}[Ch28 最小 case package]
...
results/ch28_spectral_case/
  ...
  figures/
    pca_and_review_spectra.png
\end{lstlisting}
\end{examplebox}

这份清单的作用，是让光谱分类案例成为“预处理证据 + PCA 结构审查 + 分类 baseline + 错分光谱复查”的可仿写样板。后续是否引入 1D CNN，必须以这份 baseline 和错误分析为参照。
