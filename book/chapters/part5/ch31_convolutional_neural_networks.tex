...
\begin{examplebox}[Deep Learning Model Card：二维 CNN]
\begin{itemize}
    \item 输入结构：二维图像 cutout 或多通道图像张量。
    \item 模型结构：局部卷积核、非线性激活、池化或下采样、分类/回归头。
    \item 训练目标：通常是交叉熵、MSE 或任务定义的监督损失。
    \item 学到什么：可复用的局部图像模式及其组合，例如亮核、边缘、纹理或旋臂片段。
    \item 主要失败模式：学习背景、裁剪边界、仪器模式、标签噪声或训练集选择偏差。
    \item 科学边界：可以支持“图像模式对当前标签有预测力”，不能直接证明形态结构的形成机制。
\end{itemize}
\end{examplebox}
