...
\section{本章导读：光谱里的局部模式需要沿波长轴理解}

一维卷积适合光谱，不是因为光谱可以被随便当成数组，而是因为许多光谱信息本来就沿波长轴局部出现：吸收线、发射线、线宽、双线结构、连续谱斜率的局部变化。卷积窗口可以在不同波长位置寻找相似模式，这让它比手工挑选少量线指标更灵活。

这种灵活性也带来风险。若波长校准、归一化、重采样或坏像元处理没有做好，卷积模型可能会稳定学习到处理痕迹，而不是物理谱线。也就是说，一维卷积并不会替代 Part II 的光谱理解，它只是建立在那些理解之上的表示工具。

读这一章时，请始终把卷积输出和光谱诊断联系起来：模型依赖的是哪段波长、哪些局部模式可能对应谱线、哪些响应可能只是噪声或预处理产物。
...
\begin{examplebox}[Deep Learning Model Card：一维光谱 CNN]
\begin{itemize}
    \item 输入结构：按波长排序的一维 flux window 或多通道光谱序列。
    \item 模型结构：沿波长轴滑动的一维卷积核、激活、池化和轻量分类/回归头。
    \item 训练目标：按任务选择交叉熵、MSE 或重构/对比目标。
    \item 学到什么：局部谱线形态、线宽、峰谷组合和相对位移不太敏感的模式。
    \item 主要失败模式：学习重采样痕迹、连续谱处理差异、坏像元、仪器响应或样本选择。
    \item 科学边界：可以支持“某些局部谱线模式有预测力”，不能替代波长校准、线识别和物理解释。
\end{itemize}
\end{examplebox}
