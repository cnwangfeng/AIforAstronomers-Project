...
\section{本章导读：重构得不好，可能是异常，也可能是模型没学会}

自编码器很适合讲“表示学习”这件事：模型把输入压缩到低维表示，再尝试重构原始输入。如果重构效果很好，说明低维表示保留了某些主要结构；如果重构效果很差，样本可能异常，也可能只是模型能力不足、训练样本不覆盖、预处理不一致或噪声太强。

因此，本章不会把重构误差直接讲成异常发现。它更像一个候选信号：提醒我们哪些样本不符合模型在训练集中学到的常见模式。这个信号是否有科学价值，仍然需要回到数据质量、物理背景和人工复核。

读这一章时，请把自编码器的 latent representation 看成一种工具，而不是自然规律本身。表示空间能帮助我们压缩和比较样本，但解释它之前必须先问：模型到底在哪些数据上学会了什么。
...
表~\ref{tab:ch33_mean_vs_autoencoder} 把平均谱 baseline、自编码器和局部共享版本放在同一个重构误差对照里。
...
\begin{examplebox}[Deep Learning Model Card：自编码器]
\begin{itemize}
    \item 输入结构：图像、光谱或其他高维数组。
    \item 模型结构：编码器 $\rightarrow$ 低维 latent bottleneck $\rightarrow$ 解码器。
    \item 训练目标：最小化输入与重构之间的误差。
    \item 学到什么：训练数据中最容易被压缩和重构的常见结构。
    \item 主要失败模式：训练集被异常污染、瓶颈过大导致异常也能被重构、预处理不一致导致假异常。
    \item 科学边界：重构误差可以排序候选，不能单独宣布新发现。
\end{itemize}
\end{examplebox}
...
这一步非常适合教学，因为它把“检索”和“异常检测”这两件常被混在一起的事情分开了。\textbf{前者更像是在找上下文和参照物；后者更像是在判断这种相似性是否仍然可信。}
