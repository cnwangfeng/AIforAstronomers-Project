...
\begin{examplebox}[Deep Learning Model Card：Transformer / Attention]
\begin{itemize}
    \item 输入结构：token、patch、光谱窗口或时间片段组成的序列。
    \item 模型结构：embedding、位置编码、多头注意力、前馈层、残差连接和任务头。
    \item 训练目标：分类、回归、masked prediction、next-token prediction 或其他自监督目标。
    \item 学到什么：不同位置之间的上下文关系和可迁移表示。
    \item 主要失败模式：token 化丢失科学信息、预训练数据偏差、注意力权重被过度解释、迁移任务分布不匹配。
    \item 科学边界：可以提供强表示或候选预测，不能把注意力图直接当成物理因果图。
\end{itemize}
\end{examplebox}
