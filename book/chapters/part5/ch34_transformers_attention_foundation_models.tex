...
在这个教学数据上，bag-of-tokens baseline 的测试集准确率只有 \texttt{0.33}。这并不奇怪，因为三个类别的 token 计数向量完全相同，模型失去了所有顺序信息。
...
\caption{本章教学 notebook 中的两个最小对照组。这里的重点不是追求高分，而是直接暴露顺序信息和动态读出在序列建模中的必要性。}
...
在更新后的 notebook 里，这个 masked-token workflow 分成两步。第一步仍然保留一个 hand-crafted 的位置感知 masked-token 头，直接把查询跳到 \texttt{tok06} 的 anchor token 上，用它建立“注意力究竟在读哪里”的直觉；这个版本在 clean test 上达到 \texttt{1.00}，并通过 validation 置信度阈值把 2 个 review 样本送入 \texttt{manual\_review}。

第二步再往前走一点：notebook 用 6 条 clean training rows 训练一个\textbf{纯 Python、单头、低维的 tiny self-attention masked-token learner}。它只保留 token embedding、position embedding、一个可训练 attention 头和 3 类输出投影，不依赖 NumPy、PyTorch 或大模型库。在固定随机种子下训练 50 个 epoch 后，这个 tiny learner 同样在 clean validation/test 上达到 \texttt{1.00}，并把 validation-calibrated ready threshold 校准到 \texttt{0.979}，继续把 \texttt{R\_blend} 与 \texttt{R\_mask} 两个 held-out review 样本送回 \texttt{manual\_review}。
...
\begin{itemize}
    ...
    \item 再读取第二组 masked-token workflow 数据，先写一个 hand-crafted masked-token 头，再训练一个纯 Python 的 tiny self-attention masked-token learner；
    \item 用 validation 置信度校准 ready threshold，检查这个 trainable learner 是否自己把注意力学到 \texttt{tok06}，并把 held-out 样本路由到 masked-prediction-ready 或 manual-review。
\end{itemize}

notebook 里仍然没有训练大模型，也没有堆叠多层 block。它只训练了一个单头、低维、几十个样本级更新就能收敛的 teaching-size masked-token learner。重点还是同一个：让学生先理解\textbf{注意力是一种动态读出机制，而 masked-token objective 只是把这种读出变成了一个可训练的预训练任务。}

\section{怎样从这里走向 Transformer}

在真正的 Transformer 里，上面的最小注意力头会被扩展成更完整的结构：
...
本章最重要的价值，不在于立刻训练一个 Transformer，而在于建立现代序列建模的关键直觉：当类别或任务取决于“某个位置应该去读哪一个相关 token”时，简单平均和固定局部感受野往往不够，而注意力提供了一种更自然的动态读出机制。这也是 Transformer 和科学基础模型能够跨文本、图像、光谱和时间序列统一扩展的根本原因之一。
