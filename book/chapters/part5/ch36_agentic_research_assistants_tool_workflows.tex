...
\section{本章导读：会调用工具的 AI 更需要边界}

Agentic workflow 的吸引力在于，AI 不只是回答问题，还能调用工具、读写文件、运行代码、整理结果，甚至分解任务。能力越强，边界越重要：一个会行动的助手如果没有检查点、权限边界、日志和人工接管条件，就可能把错误从文字扩展到真实文件和项目状态里。

本章讨论的重点不是“让 agent 做更多事”，而是“怎样让 agent 的每一步都可审计”。工具调用是否成功、输入材料是否允许使用、输出是否被验证、失败后是否停止或降级，这些都必须写进 workflow，而不能只依赖助手的自信语气。

读这一章时，请把 agent 看成一个需要 protocol 的研究助理。好的 agentic workflow 应该有明确任务、有限权限、可复现日志、失败处理和 human signoff。
...
因此这个 baseline 的 top-3 precision 只有 \texttt{0.33}。表~\ref{tab:ch36_baseline_vs_agentic} 把它与显式工具链 workflow 放在同一个科研分诊对照里。
...
\end{tikzpicture}
\caption{本章 notebook 中的最小多步工具链。候选体先经过质量门，再进入 novelty / crossmatch 检查和优先级评分；最终出口不是单一答案，而是 \texttt{follow\_now}、\texttt{manual\_review} 或 \texttt{reject}，这些步骤都会进入 action log。}
...
到了后续项目阶段，这种能力会变得更加重要。真实项目里，你很可能会让 AI 帮你：
...
在最终项目包里，action log 会成为 Evidence Record 和 AI Usage Log 的一部分，manual review queue 会作为 error / review artifact 进入项目包。它们共同回答一个问题：代理每一步为什么这样做，以及它在哪些边界处停下来交还给人。
