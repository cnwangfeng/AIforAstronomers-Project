...
\section{本章导读：可信 AI 使用也是科研能力的一部分}

AI 伦理、版权和科研规范不是教材最后的附加说明，而是现代科研训练的一部分。学生越早使用 LLM、agent、自动写作和生成工具，就越需要知道哪些材料可以输入、哪些输出必须核查、哪些贡献需要披露、哪些内容涉及版权或署名边界。

本章的目标不是吓退 AI 使用，而是让 AI 使用变得可持续、可公开、可审计。一个项目如果因为 AI 使用记录不清、引用不准、版权边界模糊或责任归属不明而无法发布，那么技术上跑通也没有意义。

读这一章时，请把规范看成保护项目的工程设施。它帮助你在团队协作、课程提交、公开发布和未来科研中保留可信度。
...
\begin{enumerate}
    ...
    \item 学会从一份结构化的 AI Usage Log 出发，写出更诚实、可检查的 AI-use statement，而不是事后凭印象补一句“使用过 AI”。
\end{enumerate}
...
\section{先有 AI Usage Log，再写 AI-use statement}
...
到了 后续项目里，这些内容几乎一定会重新出现。学生不仅要会“用 AI”，还要会：
...
\begin{itemize}
    \item 把分散的 use-case 记录整理成更清楚的 AI Usage Log；
