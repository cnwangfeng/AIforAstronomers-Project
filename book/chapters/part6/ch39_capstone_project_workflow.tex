...
\begin{itemize}
    ...
    \item AI 确实帮了很多忙，但最终没有留下可检查的 usage log。
\end{itemize}
...
在本章教学数据上，按 \texttt{model\_hype\_score} 排名前三的项目里，真正已经可以直接进入 capstone 执行阶段的只有一个。因此 notebook 中会得到下面这个最小对照：
...
如果项目使用了第 30--34 章的深度模型，或者第 35--38 章的 LLM / agent 工具，那么 Part V 收束章里的“可信交付包”也应该成为 project board 的一部分。深度模型至少要留下 model-card 式记录：任务、baseline、训练验证、输入预处理、主要错误和失效边界；LLM 或 agent workflow 至少要留下 usage-log 式记录：使用场景、材料边界、人工核查、失败接管和公开声明。
