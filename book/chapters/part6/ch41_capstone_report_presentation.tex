...
\section{案例导读：最终展示不是包装，而是证据重组}

学生很容易以为最后一周的任务只是把 notebook 结果复制进报告，再把图放进 slides。真正困难的地方在于，最终读者不会沿着 notebook 的试错顺序阅读，而会追问科学问题、数据、baseline、评估、错误样本、限制和 AI 使用声明是否形成一条可信链。

本章训练的是最后一公里的交付能力：把已经做过的分析重新组织成别人能理解、能追问、也能判断责任边界的报告与展示。所谓最终签字，不是形式上的“完成”，而是学生愿意为每个主要 claim 承担解释责任。
...
在本章教学数据上，按 \texttt{slide\_polish\_score} 排名前三的项目里，真正已经可以直接提交的只有一个。因此 notebook 中会看到表~\ref{tab:ch41_baseline_vs_delivery} 这个最小对照：
