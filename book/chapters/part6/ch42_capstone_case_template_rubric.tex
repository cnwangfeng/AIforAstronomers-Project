...
在本章教学数据上，只按总分取前三个项目时，可直接评分的精度是 0.67，并且混入了一个评分前阻塞项目。结构化 rubric workflow 则先检查 gate，再判断最低线和修订负担：
...
\section{Case template 应该长什么样}

如果要把这个 rubric 直接发给学生，推荐把项目包组织成下面的 case template：

\begin{enumerate}
    \item \textbf{Project card}：一句科学问题、一句话说明为什么这个问题可以用当前数据回答；
    \item \textbf{Data card}：数据来源、许可证、字段、清洗规则、train / validation / test 划分；
    \item \textbf{Baseline card}：至少一个非深度学习或简单模型 baseline；
    \item \textbf{Model card}：模型选择、输入输出、训练设置、失败样本；
    \item \textbf{Evaluation card}：指标、验证集、测试集、选择效应和过拟合检查；
    \item \textbf{Interpretation card}：关键图表、主要 claim、局限性和不确定度；
    \item \textbf{Reproducibility card}：notebook 复跑说明、环境、随机种子、Git 记录；
    \item \textbf{Integrity card}：AI-use statement、引用核查、human signoff 和公开边界。
\end{enumerate}

这八张卡片的作用，是让 Capstone 从“做了很多事情”变成“每个 claim 都知道证据在哪里”。

\section{把八张卡片接回评分维度}

为了避免 rubric 看起来像另一张独立表格，建议在学生材料中明确写出 case template 和评分维度的对应关系：

\begin{itemize}
    \item Project card 对应科学问题清晰度；
    \item Data card 和 Reproducibility card 共同对应数据处理与可复现性；
    \item Baseline card、Model card 和 Evaluation card 共同对应模型建立与评估；
    \item Interpretation card 对应结果解释、局限性和不确定度；
    \item Reproducibility card 同时支撑代码质量、Git 记录和 notebook 复跑；
    \item Integrity card 不直接加分，而是 AI 使用披露、引用核查、公开边界和 human signoff 的评分前 gate。
\end{itemize}
...
\begin{itemize}
    \item 把一个松散项目整理成八张 case-template card；
...
把你自己的 Capstone 项目整理成八张 case-template card。然后用本章 rubric 给自己做一次预评分，列出三条最重要的修订动作。
\end{exercisebox}
