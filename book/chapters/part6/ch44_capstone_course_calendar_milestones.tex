...
\section{案例导读：课程日历是风险控制工具}

Capstone 的日历不应该只是日期表。它真正要回答的是：哪些问题必须在第 3 周暴露，哪些证据必须在 midpoint 前冻结，哪些边界不能拖到 final delivery 才检查。

本章把课程节奏设计成一组质量门。学生会看到为什么 proposal、pilot、midpoint review、notebook freeze 和最终展示不能随意挪动；教师和助教则会看到如何用日历提前发现范围、复现、数据许可和 AI 使用声明的风险。
