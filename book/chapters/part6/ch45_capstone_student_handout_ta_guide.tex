...
\section{案例导读：同一份任务要被学生和助教读成同一件事}

Capstone 最怕的一种情况是：教师以为要求已经说清，学生却不知道该交什么，助教也不知道反馈应该落到哪一条 rubric 上。handout 和 TA guide 的价值，就在于把同一个项目要求翻译成学生可执行、助教可评分的共同语言。

本章关注的不是文档美化，而是沟通一致性。好的学生 handout 应该减少猜测，好的 TA guide 应该减少评分漂移，两者共同保护课程项目不被口头解释和临场经验绑架。
...
在本章教学数据上，只按发布优先级取前四项，可直接发布的精度是 0.50，政策复核项混入率也是 0.50。表~\ref{tab:ch45_priority_vs_release} 展示了结构化 release workflow 如何先检查政策，再检查助教校准，最后检查学生清晰度和样例质量：
