...
\section{案例导读：反馈只有进入修订账本，才会变成课程资产}

一次试教或一轮课程结束后，最容易丢失的不是文件，而是为什么要改、谁建议改、哪些问题已经修过、哪些问题暂时不修的判断。没有修订闭环，课程团队很快就会在下一轮重复遇到同样的问题。

本章把反馈、修订和归档连成同一条证据链。学生能看到为什么项目提交后还需要整理；教师和维护者则能看到如何把分散意见转成可追踪的 revision ledger，而不是一句含糊的“下次优化”。
...
在本章教学数据上，只按低修订成本取前四项，可归档精度是 0.75，政策复核项混入率是 0.25。表~\ref{tab:ch46_effort_vs_archive} 展示了结构化 revision / archive workflow 如何先检查共享边界，再检查复现性，最后处理证据、学生反馈和助教反馈：
