...
\section{案例导读：交接包保护的是课程记忆}

课程资源最脆弱的时刻，往往不是正在授课时，而是换教师、换助教、换学期的时候。文件可能还在，但如果没有上下文、判断理由、失败记录和下一步建议，新接手的人仍然要从头猜一遍。

本章把 instructor handoff 看成课程资产的交接协议。它不只是列文件位置，而是说明哪些证据已经可信、哪些风险仍在、哪些决策不能只看最终版本，必须回到修订和运行记录。
