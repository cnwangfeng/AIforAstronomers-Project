...
\section{案例导读：采用说明要替外部教师降低第一次使用成本}

外部教师第一次打开课程包时，最需要的不是全部细节，而是清楚入口、预计投入、适用边界、常见失败和最小采用路径。如果这些信息缺失，即使材料质量很高，也很难被真正采用。

本章把 launch QA 和 adoption note 放在一起，是因为公开发布后的第一批读者会用它们判断这套材料是否值得投入时间。好的采用说明不是宣传语，而是帮助别人做现实决策的边界文档。
...
在本章教学数据上，只按发布优先级取前四项，真正可发布的精度只有 0.25，未就绪材料混入率是 0.75。表~\ref{tab:ch49_priority_vs_launch_qa} 展示了结构化 launch-QA workflow 如何先检查链接，再检查 smoke test，再看支持边界，最后补采用说明：
