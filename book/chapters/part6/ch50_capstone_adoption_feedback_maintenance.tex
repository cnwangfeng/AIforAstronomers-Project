...
\section{案例导读：发布后的反馈是维护信号，不是背景噪声}

课程包被别人采用之后，反馈会从很多方向进入：学生运行失败、教师改动日历、助教调整 rubric、外部环境更新依赖。如果没有维护节奏，这些反馈会堆成零散消息，最后没人知道哪些该修、哪些该解释、哪些该拒绝。

本章训练的是发布后的课程维护能力。它要求把 adoption feedback route 到清楚出口，并为下一次修订安排责任人和时间，而不是只凭热情临时响应。
