...
\section{案例导读：最诱人的外部项目也需要先过准入边界}

外部合作可以带来真实数据、真实问题和真实导师，但它也可能带来未发表材料、时间承诺、署名期待、学生隐私、工具访问和成果发布边界。越是看起来有价值的机会，越需要在进入课堂前说清楚。

本章把 guest intake 看成 Capstone 的外部边界门。它要求课程团队先评估数据权限、工作量、支持责任、学生收益和退出条件，再决定某个合作是否适合作为课程项目。
...
在本章教学数据上，只按合作价值取前四项，真正可准入的精度只有 0.25，未就绪项混入率是 0.75。表~\ref{tab:ch59_value_vs_guest_intake} 展示了结构化 intake workflow 如何先看 boundary，再看 host owner，再检查 scope 和 guest material：
