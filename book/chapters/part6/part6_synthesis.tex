...
\section*{一页总索引应该压住哪些重复}

如果后续继续打磨 Part VI，优先不要再增加新的运行章节，而是把重复出现的检查项压回一页总索引。这个总索引至少应包含四列：

\begin{enumerate}
    \item \textbf{artifact}：学生项目包、助教评分包、教师交接包、公开发布包或长期维护包；
    \item \textbf{gate}：scope、evidence、boundary、owner 或 closure 中哪一类还没过；
    \item \textbf{route}：ready、review、revise、blocked、assign owner、sync dependency 或 close blocker；
    \item \textbf{next action}：下一步由谁在什么时间前补哪份证据。
\end{enumerate}

有了这四列，后半部分的章节就不必被理解成二十多个互不相干的 checklist，而是同一张课程资产状态表在不同场景下的展开：评分时看 rubric，发布时看 release，交接时看 handoff，重启时看 preflight，长期维护时看 ledger。
